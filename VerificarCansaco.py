...
def calculo_mar(face, p_boca):
    try:
        ...
        mar = (np.linalg.norm(face_boca[0] - face_boca[1]) + np.linalg.norm(face_boca[2] - face_boca[3]) +
               np.linalg.norm(face_boca[2] - face_boca[3]) + np.linalg.norm(face_boca[4] - face_boca[5])) / (
        ...
        try:
            for face_landmarks in saida_facemesh.multi_face_landmarks:
                ...
                if piscadas_pm < 10 or tempo >= 1.5:
                    ...
                    cv2.putText(frame, f"Pode ser que você esteja com sono,", (60, 420),
                                cv2.FONT_HERSHEY_DUPLEX,
                                0.85, (58, 58, 55), 1)
                    cv2.putText(frame, f"considere descansar.", (180, 145),
                                cv2.FONT_HERSHEY_DUPLEX,
                                0.85, (58, 58, 55), 1)
